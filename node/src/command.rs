...
use sc_cli::{
    ChainSpec, CliConfiguration, DefaultConfigurationValues, Error, ImportParams, KeystoreParams,
    NetworkParams, Result, RuntimeVersion, SharedParams, SubstrateCli,
};
use sc_executor::error::WasmError;
use sc_service::{
...
pub fn run() -> Result<()> {
    ...
    match &cli.subcommand {
        ...
        Some(Subcommand::ExportGenesisWasm(params)) => {
            ...
            let wasm: Vec<u8> = if params.decompress {
                sp_maybe_compressed_blob::decompress(raw_wasm_blob.as_slice(), CODE_BLOB_BOMB_LIMIT)
                    .map(|c| c.into_owned())
            ...
            let output_buf = if params.raw {
                ...
                format!("0x{:?}", HexDisplay::from(&wasm)).into_bytes()
            };
